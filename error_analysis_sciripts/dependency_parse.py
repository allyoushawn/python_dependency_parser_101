...
def get_root_distance(current_node, gold_heads):
    ...
    start = False
    while True:
        if head == len(gold_heads) - 1 and start == True:
            break
...
def main(model_dir, train_loc, heldout_in, heldout_gold):
    ...
    for (words, tags), (_, _, gold_heads, gold_labels) in zip(input_sents, gold_sents):
        _, heads = parser.parse(words)
        for i, w in list(enumerate(words))[1:-1]:
            dist = get_root_distance(i, gold_heads)
            if gold_labels[i] in ('P', 'punct'):
                continue
            if heads[i] == gold_heads[i]:
                c += 1
            ...
            t += 1
            total[dist] += 1
    with open('self_parser_hit.pkl', 'wb') as fp:
        pickle.dump(hit, fp)
    with open('self_parser_total.pkl', 'wb') as fp:
